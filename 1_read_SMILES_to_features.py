...
def read_smiles(csv_file, smiles_column="Smiles_1"):
...
def generate_features(csv_file, output_csv):
    ...
    df["CDK_Features"] = df["Smiles_1"].apply(compute_cdk_features)
    ...
    final_df = pd.concat([df["Smiles_1"], rdkit_df, mordred_df, pybiomed_df, cdk_df], axis=1)
    # Save results
    ...
    for i, row in df2.iterrows():
        for col in df2.columns:
            try:
                ...
                if isinstance(row[col], str) and col != "Smiles_1":  # Exclude SMILES column
                    df2.at[i, col] = None
    # Merge on 'SMILES', keeping the order of the first file
    merged_df = df1.merge(df2, on="Smiles_1", how="left")
